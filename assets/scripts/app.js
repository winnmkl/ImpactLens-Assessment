...
alasql(`CREATE TABLE IF NOT EXISTS Assets (
    ...
    owner STRING, user_name STRING, custodian STRING, description STRING, 
...
if (isInitialized) {
    // If they have been here before, strictly load their saved data (even if it's empty!)
...
function saveAssetToDB() {
  ...
  alasql(`INSERT INTO Assets VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, [
      ...
      g('f-desc'), g('f-pii'), g('f-spi'), g('f-corp'),
...
function clearForm() {
  const fields = ['f-name','f-group','f-owner','f-user','f-custodian','f-desc','f-risk-desc','f-action-plan','f-action-owner','f-action-date','f-risk-category'];
...
function exportDataXLSX() {
    ...
    const headers = [ "Asset ID", "Name of Information Asset", "Description", "Group", "Owner", "User", "Custodian", "Type", "PII", "SPI", "Corp Info", "C", "I", "A", "Valuation", "Class", "Risk", "Prob", "Sev", "Inherent", "Control 1", "Control 2", "Control 3", "Control 4", "Control 5", "Control 6", "Control 7", "Control 8", "Control Effect", "Residual", "Action Plan", "Action Owner", "Target Date" ];

    const dataRows = dbAssets.map(a => {
        const ctrls = alasql("SELECT ctrl_id FROM AssetControls WHERE asset_id = ?", [a.id]).map(r => r.ctrl_id);
        return [
            a.id, a.name, a.description, a.group_name, a.owner, a.user_name, a.custodian, a.type, a.pii, a.spi, a.corp,
